...
function FormCreate({ logged, setShowModalCreateProduct, showModalCreateProduct, form, setShowFormCreate, getProductAfterCreate, productNewCreate }) {
	...
	const onSubmit = async (e) => {
		...
		try {
			...
			await getProductAfterCreate(logged?.empresa_id, response.producto_id);
	...
	useEffect(() => {
		if (showModalCreateProduct && logged) {
			...
			form.setFieldsValue({ codigo_barras: productNewCreate.barcode !== '999999' ? productNewCreate.barcode : null });

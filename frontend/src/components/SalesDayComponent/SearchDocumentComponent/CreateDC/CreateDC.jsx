...
function CreateDC({ showModalCreateDC, setShowModalCreateDC, document, setCurrentDocument, logged }) {
	...
	const addProduct = () => {
		...
		const documentoDetalle = {
			...
			"url_foto": null,
			"impuesto_producto": find.impuesto,
			"saldo": null,
			"peso_cotero": null,
			"cotero_id": null
		}
	...
	const removeProduct = (id) => {
		...
		setTableFill(tableFill.map(item => {
			if (item.documento_detalle_id === id) {
	...
	useEffect(() => {
		if(showModalCreateDC) {
			if(document?.tipo_documento_id == 4) {
				toast.warning('No puedes realizar un nota credito debito a una cotización');
	...
	const handleProducts = () => {
				...
				tableFill.forEach(p => { //Recorro los productos actuales.
					...
					if(findProd) { // Si el producto existe
						productsModified.push({
							...findProd, //Guardamos todo el producto
							cantidad: Number(findProd.cantidad) + Number(oldCantidad) - Number(p.cantidad)
							//Y a su cantidad decimos que es mayor a la antigua menos la actual.
						});
					}
				});
			}
		...
		if(document?.tipo_documento_id == 10) { //Si es una factura de venta. 
			if(tableFill.length) {
				tableFill.forEach(p => { //Recorro los productos actuales.
					const findProd = productsOpt.find(el => el.producto_id == p.producto_id); //Busco el producto
					const oldCantidad = p.oldCantidad ? p.oldCantidad : p.cantidad;
					...
					if(findProd) { // Si el producto existe
						productsModified.push({
							...findProd, //Guardamos todo el producto
							cantidad: p?.documento_detalle_id ? 
								Number(findProd.cantidad) + Number(oldCantidad) - Number(p.cantidad) : 
								Number(findProd.cantidad) - Number(p.cantidad)
							//Y a su cantidad decimos que es mayor a la antigua menos la actual.
						});
					}
				});
		...
		if(document?.tipo_documento_id == 9) { //Si es un nro de guía. 
			const productsDifference = getDifference(documentoDetallesInit, tableFill, 'producto_id');
			//Buscamos los productos diferentes a los actuales, es decir, los eliminados.
			if(productsDifference.length) { //Si encontramos..
				productsDifference.forEach(pd => { //Recorremos..
	...
	const verifyDocument = async () => {
		...
		try {
			...
			if(response) {
				...
				if(response[0]?.cufe === "")
				{
					toast.warning('No se puede crear la nota de una fatura que no ha sido enviada de manera electronica.');
					return;
				}

				const doc = parseFloat(total) > parseFloat(document?.total) ? 'ND' : 
				parseFloat(total) < parseFloat(document?.total) ? 'NC' : 'N/A'
				// console.log(cufe)
				...
				const item = {
					...
					cliente_id: document?.cliente_id || 1,
					...
					iva_5: vrIva5,
					iva_19: vrIva19,
					letra_consecutivo: `${resolutions?.letra_consecutivo}${doc}`,
					descripcion_trabajador: description,
					resolucion_empresa_id: document?.resolucion_empresa_id || null,
					consecutivo_dian: response[0]?.consecutivo_dian || null,
					impreso: 1,
					invoice_id: 1,
					cufe: response[0]?.cufe ?? "",
					qrcode: response[0]?.qrcode ?? "",
				...
				let updtDocument = {
					...document,
					nota_id: doc == 'NC' ? newDocument?.documento_id : null,
				}
				await updateDocumentoApi( updtDocument );
				...
				return (
					<>
						<Button 
							type='default'
							size='small'
							onClick={ (e) => removeProduct(record.documento_detalle_id) }
						>
							<DeleteOutlined />
						</Button>
					</>
	...
	return (
		...
		>
			...
			<Table 
				columns={columns}
				dataSource={tableFill.filter(item => item.estado === 1)}
				rowKey="producto_id"
